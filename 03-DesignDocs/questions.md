...
> **Q-12／Q-13 已於 PRD v3.10 裁定、Q-15 已於 PRD v3.11 裁定**（2026-09-04 補記結果）。<br>
> **唯一待處理：Q-14（🔴 上線阻擋，需 CEO 在 Vercel 設環境變數，repo 內無法滿足）。**
...
| **裁定結果**（2026-09-04 補記）| 技術總監於 **PRD v3.10 §7.5c** 採納 ①：`max_tokens: 4096` + `stop_reason === "max_tokens"` 記 log。實作（`api/parse-post.js`）本就如此。**設計文件 §6.3 的 pseudo-code 直到 2026-09-04 才跟上** —— v3.2.0 回寫時把說明段改對了，卻漏掉同一節那段給人照抄的骨架，於是文件裡 2048 與 4096 並存了一天。<br>**本表「問題」欄的算式已被上游修正措辭**（PRD §7.5c，2026-09-03 由程式審查員指出，PR #21）：§7.3 的 tool schema **只對 `note` 寫「60 字內」**，`name` / `nameJa` / `area` 既無長度規定也無 `maxLength`，所以「12 筆 × 四欄位」只是**量級參考**，不是輸出上界的保證。**真正的安全網是 `stop_reason === "max_tokens"` 的檢查，不是 4096 這個數字。** |
...
## Q-15 ✅ **已裁定（PRD v3.11 §7.1／§7.4，採納 ①）** — `upstream_error` 未列於 PRD §7.1 的錯誤碼清單
...
| **裁定結果**（2026-09-04 補記）| 技術總監於 **PRD v3.11** 採納 ①：§7.1 的 `reason` union 補上 `upstream_error`；§7.4 另立一列寫下**為什麼只有這一種可以安全抽出** —— 它在 trip key 檢查之後，抽出不洩漏存在性，而 `rate_limited` 必須繼續由「限流」與「trip 不存在」共用且 `message` 逐字相同。那一段是刻意留的護欄：下一個看到「`rate_limited` 一碼二用」的人，直覺會想順手把它也拆開，那會重新打開 Q-13 修掉的存在性探測破口。設計文件（後端 §6.5、cross-check §0c-17／§3、commits-plan §0c-7）已於同日回寫，四者一致。 |
| **備註** | 開發員**不修改 `01-PRD/`**，本題的 PRD 改動由技術總監於 PR #23 完成。 |
