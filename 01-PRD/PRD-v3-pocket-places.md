...
### 4.4b S-07「自己加的地點」退出 MVP（2026-09-03 裁定，附更正）

**裁定**：`place.pocketId === ""` 的手動地點（UI spec S-07）**退出 MVP**，相關死碼移除，
`pocketId` 欄位保留在 schema 中。

**⚠ 裁定當下我給的理由是錯的，在此更正** —— 開發員在實作階段指出：

> 我原本寫「新增一整條手動加地點的 UI 流程屬於範圍蔓延」。**這句話不成立。** 入口早就存在
> （S-13 解析失敗訊息條下方的「自己輸入一個地點」），要讓 S-07 成真只需要在 `commitReview`
> 加一個分支，大約一行。我用「成本太高」駁回了一個其實很便宜的東西。

**正確的理由（本節以此為準）**：逃生口發生在**解析失敗之後**，此時使用者手上多半仍有來源連結。
把它存成真正的 pocket，才留得住「之後回去看原貼文」這件事 —— 那是這個功能的核心價值之一。
所以實作的選擇是**對的**，不是**便宜的**。

**為什麼要特別更正**：用「成本」當理由，下一輪就會有人說「現在便宜了」把它加回來，
而那個決定的真正依據（保留來源連結）會被一起丟掉。**理由寫錯比沒寫更危險。**

**連帶待處理**（開發員指出、本裁定造成的後果）：一般模式下使用者可以只貼文字不貼連結 →
解析失敗 → 逃生口 → 產生一則 `sourceUrl: ""`、標題「收藏的貼文」的 pocket。
**那不是貼文，是使用者自己打的字，標題在說謊。** S-07 的「📌 自己加的地點」標籤原本正是為此而存在。
需 UI/UX 補一條規則：`sourceUrl` 為空時標題換字。屬 `02-Design/`，不在開發員權限內。

**未來復活時的閘門**：見 `03-DesignDocs/merge-gates.md` 的 **G-04** —— 保留欄位不等於保留安全。
...
> 是**輸出上界**。以 `clampPlaces` 的截斷值估算（12 筆 × 約 220 個中日文字 ≈ 2,640 字，
> 加 `title` / `summary` 與 JSON 結構開銷）滿載約 **3–4k tokens**，2048 會在最壞情況截斷，
> 而被截斷的 `tool_use.input` 是**不完整但看起來合法的 JSON**，會靜默通過。
>
> **但這個算式只是量級參考，不是保證**（2026-09-03 由程式審查員指出並修正措辭）：
> §7.3 的 tool schema 只對 `note` 寫「60 字內」，`name` / `nameJa` / `area` 既無長度規定也無
> `maxLength`，所以模型實際可以回得更長 —— `clampPlaces` 是**收到之後**才截斷的。
> 因此 **真正的安全網是 `stop_reason === "max_tokens"` 的檢查，不是這個數字**。
> 4096 只是把「撞到上限」從常見變成罕見；檢查則保證撞到時不會無聲無息。
>
